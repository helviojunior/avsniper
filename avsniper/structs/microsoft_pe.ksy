...
types:
  ...
  pe_header:
    seq:
      ...
      - id: coff_hdr
        type: coff_header
      - id: save_offset_of_optional_hdr
        size: 0
        if: offset_of_optional_hdr < 0
      - id: optional_hdr
    ...
    instances:
      ...
      certificate_table:
        ...
        if: optional_hdr.data_dirs.certificate_table.size > 0
      resources_table:
        pos: optional_hdr.data_dirs.resource_table.pointer_to_raw_data
        size: optional_hdr.data_dirs.resource_table.size
        type: resource_directory_table(directory_entry_type::undefined)
      ...
      debug_directory:
        pos: optional_hdr.data_dirs.debug.pointer_to_raw_data
        size: optional_hdr.data_dirs.debug.size
        type: debug_directory
        if: optional_hdr.data_dirs.debug.size > 0
  coff_header:
  ...
  optional_header:
    ...
    instances:
      pointer_to_raw_data:
        value: _parent.offset_of_optional_hdr + _io.pos
  optional_header_std:
    seq:
      - id: format
        type: u2
  ...
  optional_header_data_dirs:
    ...
    instances:
      pointer_to_raw_data:
        value: _parent.pointer_to_raw_data + _io.pos
  data_dir:
    to-string: |
  ...
  section:
    ...
    seq:
      - id: save_offset
        size: 0
        if: pointer_to_section_header < 0
      - id: name
        type: str
        encoding: UTF-8
        size: 8
  ...
  certificate_entry:
    ...
    to-string: |
      'Certificate Entry <Type: ' + certificate_type.to_i.to_s + ', Length: ' + length.to_s + ', PointerToRawData: ' + pointer_to_raw_data.to_s + '>'
    enums:
    ...
    instances:
      offset:
        value: _io.pos
      pointer_to_raw_data:
        value: _root.pe.optional_hdr.data_dirs.certificate_table.virtual_address + offset + 8
  ...
  resource_directory_table:
    ...
    seq:
      ...
      - id: items
        ...
        type: resource_directory_entry(parent_type)

    instances:
      pointer_to_header:
        value: >
          _root.pe.optional_hdr.data_dirs.resource_table.pointer_to_raw_data + _io.pos
      has_entries:
        value: (number_of_named_entries + number_of_id_entries) > 0

  resource_directory_entry:
    to-string: |
      'Res entry <Type: ' + name_type.to_i.to_s + ', Name Addr: ' + name_address.to_s + ', Dir Addr: ' + directory_address.to_s + ', PointerToRawData: ' + pointer_to_raw_data.to_s + ', Virtual Addr: ' + virtual_address.to_s + ', Data Size: ' + data_size.to_s + '>'
    params:
      - id: parent_type
        type: u4
        enum: directory_entry_type
    ...
    instances:
      ...
      directory_table:
        ...
        type:
          ...
          cases:
            directory_entry_type::undefined: resource_directory_table(name_type)
            _: resource_directory_table(parent_type)
        if: is_directory
      virtual_address:
        pos: directory_address
        type: u4
      ...
      pointer_to_raw_data:
        value: >
          is_data_entry ?
          ( _root.pe.optional_hdr.data_dirs.resource_table.sections_lookup.section.pointer_to_raw_data +
              (virtual_address - _root.pe.optional_hdr.data_dirs.resource_table.sections_lookup.section.virtual_address)
            ):
      ...
      data:
        ...
        type:
          switch-on: parent_type
          cases:
            'directory_entry_type::group_cursor2': icon_group
            'directory_entry_type::group_cursor4': icon_group
            _: bin
        if: is_data_entry

  icon_group:
    doc-ref: https://en.wikipedia.org/wiki/ICO_(file_format)
    to-string: |
      'Icon Group <Type: ' + image_type.to_i.to_s + ', Entries: ' + number_of_entries.to_s + '>'
    seq:
      - id: reserved
        type: u2
      - id: image_type
        type: u2
        enum: icon_group_entry_type
      ...
      - id: items
        ...
        #repeat: until
        #repeat-until: '_io.size - _io.pos < 14'
        type: icon_directory_entry
        eos-error: false


  icon_directory_entry:
    doc-ref: https://en.wikipedia.org/wiki/ICO_(file_format)
    to-string: |
      'Icon Entry <w: ' + width.to_s + ', h: ' + height.to_s + ', Reference ID: ' + ordinal_id.to_s + '>'
    seq:
      - id: width
        type: u1
      - id: height
  ...
  debug_directory:
    to-string: |
      'Debug dir <address_of_raw_data: ' + address_of_raw_data.to_s + ', pointer_to_raw_data: ' + pointer_to_raw_data.to_s + '>'
    seq:
      - id: characteristics
        type: u4
      - id: time_date_stamp
